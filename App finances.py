...
# --- TAB 2: PLAN DE AHORRO AMB ESTALVI TOTAL ---
with tab2:
    if not is_ready:
        st.warning("Completa los datos en la pestaña 1")
    else:
        # Lògica de dades abans de dibuixar (Per calcular el total)
        # Internet
        p_nou_int = 20 # Basat en 300Mb
        estalvi_int = max(0, (int_val - p_nou_int) * 12)
        
        # Llum (Consum mitjà 3000 kWh/any)
        p_nou_llu = 0.11
        kwh_usuari = 0.18 # Valor per defecte si no el canvia
        estalvi_llu = max(0, (kwh_usuari - p_nou_llu) * 3000)
        
        # Mòbil
        p_nou_mob = 7
        estalvi_mob = max(0, (mob_val - p_nou_mob) * 12)
        
        # Gas
        p_nou_gas = 18
        estalvi_gas = max(0, (gas_val - p_nou_gas) * 12)
        
        # Seguro (Simulació base)
        p_nou_seg = 18
        estalvi_seg = max(0, (seg_val - p_nou_seg) * 12)
        
        estalvi_total_anual = estalvi_int + estalvi_llu + estalvi_mob + estalvi_gas + estalvi_seg

        # Banner d'Estalvi Total
        st.markdown(f"""
            <div class='total-save-banner'>
                {t['total_ahorro_msg']} {round(estalvi_total_anual, 2)} € 💰
            </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("🚗 Buscador de Seguros de Coche", expanded=True):
            ...
            with c1: marca = st.selectbox(t["f_marca"], ["Seat", "Toyota", "Otros"]); edat = st.number_input(t["f_edad"], 18, 90, 30)
            with c2: comb = st.selectbox(t["f_combustible"], ["Gasolina", "Diésel"]); carnet = st.number_input(t["f_carnet"], 0, 70, 10)
            with c3: tipo_seg = st.selectbox("Cobertura", ["Terceros", "Todo Riesgo"])
            
            preu_base = 18 if tipo_seg == "Terceros" else 45
            ca, cb, cc = st.columns(3)
            with ca: draw_offer("Qualitas", preu_base, seg_val, "https://google.com")
        ...
        with col_r:
            with st.expander("💡 Luz"):
                kwh_input = st.number_input(t["preu_kwh_actual"], 0.0, 0.5, 0.18)
                c1, c2, c3 = st.columns(3)
                with c1: draw_offer("Octopus", 0.11, kwh_input, "https://google.com", "€/kWh", True)
                with c2: draw_offer("Naturgy", 0.13, kwh_input, "https://google.com", "€/kWh", True)
                with c3: draw_offer("Endesa", 0.14, kwh_input, "https://google.com", "€/kWh", True)
...
with tab3: st.info("Próximamente: Inversión")
with tab4: st.info("Próximamente: Educación")
